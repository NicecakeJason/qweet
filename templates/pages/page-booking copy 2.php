...
// Проверяем тип поста для апартамента
$expected_post_types = array('apartament', 'flat', 'room'); // Укажите все возможные типы
if ($apartament_id > 0) {
    $post_type = get_post_type($apartament_id);
    if (!in_array($post_type, $expected_post_types)) {
        // Если тип поста неверный, перенаправляем на страницу с сообщением об ошибке
        wp_redirect(add_query_arg('booking_error', 'invalid_apartament_type', get_permalink()));
        exit;
    }
}
...
<div class="section booking-section">
    <div class="container">
        ...
        if (isset($_GET['booking_error'])) {
            ...
            } elseif ($booking_error === 'invalid_apartament_type') {
                echo '<div class="booking-block" style="background-color: #fff3f3; border: 1px solid #ffcaca; margin-bottom: 20px;">
                <div class="block-content">
                    <p style="color: var(--danger-color); font-weight: 500; margin: 0;">Выбранный объект размещения не является апартаментом или не найден. Пожалуйста, выберите корректный апартамент.</p>
                </div>
            </div>';
            }
        }
